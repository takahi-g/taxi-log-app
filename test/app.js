...
const APP_UPDATE_INFO = {
    version: "20260806_1338",
    date: "08/06 13:38",
    title: "🎉 アップデートのお知らせ (Ver: 08/06 13:38)",
    details: [
...
function updateAnalytics() {
    ...
    wdayStats.forEach(stat => {
        ...
        if (stat.daysCount > 0) {
            if (pct >= 90) {
                barColor = 'linear-gradient(90deg, #30d158, #248a3d)'; // 超得意 (緑系)
                statusBadge = '超得意';
            } else if (pct >= 80) {
                barColor = 'linear-gradient(90deg, #0a84ff, #30d158)'; // 得意 (青緑系)
                statusBadge = '得意';
            } else if (pct >= 65) {
                barColor = 'linear-gradient(90deg, #ff9f0a, #0a84ff)'; // 普通 (オレンジ・青系)
                statusBadge = '普通';
            } else {
                barColor = 'linear-gradient(90deg, #ff453a, #ff9f0a)'; // 苦手 (赤・オレンジ系)
                statusBadge = '苦手傾向';
            }
        }
...
function openGoalEditModal() {
    ...
    if (mSets && mSets.customGoals && mSets.customGoals[selectedDate] !== undefined) {
        todayTargetNet = mSets.customGoals[selectedDate];
    }
